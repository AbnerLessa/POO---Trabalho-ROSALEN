...
public class JavaApplication1 {
    public static void main(String[] args) {
        Oraculo orach = new Oraculo(); //objeto oraculo e variaveis criadas para o game
        boolean verify = true;// Variavel para verificar se ja foi ganho vida extra por fase
        boolean chance;// Variavel para retornar resultado das suplicas de vida extra
        boolean endGame = false; //variavel para caso o jogador morra na primeira fase o jogo termina
        InOut.MsgDeInformacao("GAME", "Bem vindo ao desafio do Joao Java.\nBoa Sorte!!!");
        orach.setOrachName();//Nomear oraculo
        orach.guer.setGuerName();//nomear guerreiro
        int vidas = orach.setVidas();//Instanciar vida guerreiro
        InOut.MsgDeInformacao("Prologo", orach.prologoIntroducao1());
        ...
        while (true){//colocar o jogo em estrutura de repetição para a pessoa poder jogar denovo
            ...
            while(true){//While da primeira fase
                ...
                if(vidas <= 0){//quando as vidas acabarem ele entra nessa verificação
                    if(verify){
                        chance = orach.decidirVidaExtra(orach.guer.vidaExtra());//Pelo guerreiro ter sido criado pelo oraculo, pode ser só chamado pelo objeto orach
                        if(chance){
                            InOut.MsgDeAviso("Oraculo", "Com uma vida tu retornas, uma chance ganharas!");
                            vidas = 1;
                            verify = false;//para negar proxima vida extra
                        }
                        ...
                        InOut.MsgDeInformacao("Oraculo", orach.prologoPerdedor());
                        endGame = true;//ativa o fim do jogo
            ...
            if(endGame){//caso voce tenha morrido na primeira fase, quebra o while do jogo sem opção de jogar denovo
                break;
            }
            else{
                verify = false;//reseta a variavel de verificação de vida extra, pode ter 1 por fase
                InOut.MsgDeInformacao("Oraculo", orach.prologoIntroducao2());
                ...
                while(true){//while da fase 2
                    if(vidas <=0){//Mesma verificação de vidas e suplica da fase 1, caso queira ler volte no código
                        if(verify){
                            chance = orach.decidirVidaExtra(orach.guer.vidaExtra());
                            if(chance){
                                InOut.MsgDeAviso("Oraculo", "Com uma vida tu retornas, uma chance ganharas!");
                                verify = false;
                                vidas = 1;
                        ...
                        int escolha = orach.loadLeve2(InOut.leInt("Escolha: \n[0]-impar\n[1]-par!"));
                        if(escolha == 0){//caso a escolha seja fora do desejado perde todas as vidas 
                            ...
                            InOut.MsgDeAviso("Oraculo", orach.prologoVencedor());
                            InOut.MsgDeInformacao("Historia", "O Oraculo tirou todas as vidas de " + orach.guer.getNome() + " para seu plano astral.\n"
